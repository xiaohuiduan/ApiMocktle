...
import { type FlowEdge, type FlowNode, type FlowNodeType, FlowNodeType as NT, type NodeExecStatus } from '../types/flow.types'
import type { MockCallLog, MockRule } from '../types/mock.types'
...
export function useFlowExecution() {
  ...
  const executeFlow = useCallback(async (
    ...
    const nodeRequests: Record<string, Record<string, unknown>> = {}
    const nodeResponses: Record<string, Record<string, unknown>> = {}
    ...
    const executeNode = async (node: FlowNode): Promise<{ handleId?: string, error?: boolean }> => {
      ...
      // 节点被禁用时跳过，但不中断后续流程
      if (!(nodeData.enabled as boolean)) {
      ...
      try {
        switch (nodeType) {
          ...
          case NT.SetVariable: {
            const assignments = nodeData.assignments as { variable: string, operator: string, value: string }[] || []
          ...
          case NT.HttpRequest: {
            ...
            // 存储请求/响应用于节点详情展示
            nodeRequests[nodeId] = result.data.request
            ...
            // 运行 extractors
            const extractors = nodeData.extractors as Record<string, unknown>[] | undefined
            ...
            // 运行 assertions
            const assertions = nodeData.assertions as Record<string, unknown>[] | undefined
            let assertionResults: { passed: boolean, error?: string, actual?: unknown, assertion?: Record<string, unknown> }[] = []

            if (assertions && assertions.length > 0) {
              try {
                const assertResult = await invoke<{ ok: boolean, data?: { passed: boolean, error?: string, actual?: unknown, assertion?: Record<string, unknown> }[], error?: string }>(
                  'execute_assertions',
                  {
            ...
            if (!allAssertsPassed) {
              ...
              Reflect.deleteProperty(nodeErrors, nodeId)
            }
            ...
            logs = addLog(logs, nodeId, nodeName, nodeType, nodeStatus, statusMsg, {
              durationMs: dur,
              requestJson: result.data.request,
          ...
          case NT.Assert: {
            ...
            // 1. 执行变量断言规则
            const varAssertions = nodeData.assertions as { variable?: string, operator?: string, expected?: string }[] | undefined
                  ...
                  if (branchResult.error) {
                    ...
                    for (const nn of nextNodes) {
                      if (nodeStatuses[nn.id] === 'idle') {
                        ...
                        logs = addLog(logs, nn.id, (nn.data as Record<string, unknown>)?.label as string || nn.id, nn.type, 'skipped', reason)
                      }
                    }
          ...
          case NT.SubFlow: {
            ...
            for (const n of subFlowGraph.nodes) {
              Reflect.deleteProperty(nodeStatuses, n.id)
              Reflect.deleteProperty(nodeErrors, n.id)
              Reflect.deleteProperty(nodeDurations, n.id)
            }
    ...
    const executeGraph = async (
      ...
      // 初始化子流程变量
      Object.keys(variables).forEach((k) => Reflect.deleteProperty(variables, k))
      Object.assign(variables, initVars)
      Object.keys(variableSources).forEach((k) => Reflect.deleteProperty(variableSources, k))
      ...
      Object.keys(nodeStatuses).forEach((k) => Reflect.deleteProperty(nodeStatuses, k))
      Object.assign(nodeStatuses, savedStatuses)
      Object.keys(nodeErrors).forEach((k) => Reflect.deleteProperty(nodeErrors, k))
      Object.assign(nodeErrors, savedErrors)
      Object.keys(nodeDurations).forEach((k) => Reflect.deleteProperty(nodeDurations, k))
        ...
        if (failFast && result.error) {
          ...
          for (const n of nodes) {
            if (nodeStatuses[n.id] === 'idle') {
              ...
              logs = addLog(logs, n.id, (n.data as Record<string, unknown>)?.label as string || n.id, n.type, 'skipped', reason)
            }
          }

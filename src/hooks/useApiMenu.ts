...
): { name: string, type: string, required: boolean, children?: Record<string, unknown>, items?: Record<string, unknown> }[] {
...
function formatOpenApiParams(parameters: unknown, schemaMap: SchemaMap): string | undefined {
  ...
  for (const section of ['path', 'query', 'header'] as const) {
    const rawItems = params[section] as Record<string, unknown>[] | undefined
